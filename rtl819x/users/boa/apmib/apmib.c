...
//			apmib_get(MIB_L2TP_PAYLOAD_LENGTH, (void *)&payload_len);
		...
		if (id == MIB_BT_LINK_INFO)
		{
			memcpy( (unsigned char *)value, (unsigned char *)(((long)pMibTbl) + pTbl[i].offset), 110);
		}
		else
...
#endif
		if (id == MIB_BT_LINK_INFO){
                        memcpy((unsigned char *)(((long)pMibTbl) + pTbl[i].offset), (unsigned char *)value, 110);
		}
		else 
