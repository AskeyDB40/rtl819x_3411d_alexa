...
//	unsigned char buff[sizeof(APMIB_T)+sizeof(checksum)+1];
	...
	int vlanIdx = 0;
#if defined(CONFIG_APP_APPLE_MFI_WAC) || defined(CONFIG_APP_WIFIAUDIO)
	unsigned char tmp1[16];	
	int Mib_init=apmib_init();
#endif	
...
*/
	...
	for (idx=0; idx<NUM_WLAN_INTERFACE; idx++) {
		...
		pMib->wlan[idx][i].p2p_op_channel = 11;		/*per channel that AP support */	
			...
			if(0 == idx) {
				...
				if(Mib_init ==1){
					...
					MAC_sum_complement = -(tmp1[3] + tmp1[4] + tmp1[5]);
					sprintf(mysp, "@RSC-%02X%02X%02X00%02X", tmp1[3], tmp1[4],tmp1[5], (MAC_sum_complement & 0xff));
					sprintf((char *)pMib->wlan[idx][i].ssid, "%s",  mysp);
				}else{
			...
			if(1 == idx) {				
				if (flash_read(tmp1,HW_WLAN_SETTING_OFFSET + sizeof(struct hw_wlan_setting) * idx+HW_SETTING_OFFSET,6) != 0){
					sprintf((char *)pMib->wlan[idx][i].ssid, "RTK 2.4G WiFi audio_%02X%02X%02X VAP%d", tmp1[3], tmp1[4], tmp1[5],i);
            		}else{
					printf("[%s]:%d, get hw_nic0_addr error\n",__func__,__LINE__);
				}
			}
...
#endif			
			  {
		...
		#if defined(CONFIG_APP_WIFIAUDIO)
						if(Mib_init ==1){
							flash_read(tmp1,HW_WLAN_SETTING_OFFSET + sizeof(struct hw_wlan_setting) * idx+HW_SETTING_OFFSET,6);
						
						#if defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
							sprintf((char *)pMib->wlan[idx][i].ssid, "RTK 5G Audio Config AP_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
						#else
							...
							strcpy((char *)pMib->wlan[idx][i].ssid, "RTK 11n AP 5G" );
		#else				
						strcpy((char *)pMib->wlan[idx][i].ssid, "RTK 11n AP 5G" );
        		#endif
					}else if(1 == idx)
		#if defined(CONFIG_APP_WIFIAUDIO)
					{
						flash_read(tmp1,HW_WLAN_SETTING_OFFSET + sizeof(struct hw_wlan_setting) * idx+HW_SETTING_OFFSET,6);
						#if defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
							sprintf((char *)pMib->wlan[idx][i].ssid, "RTK 2.4G Audio Config AP_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
						#else
				                            MAC_sum_complement = -(tmp1[3] + tmp1[4] + tmp1[5]);
				                            sprintf((char *)pMib->wlan[idx][i].ssid, "@RSC-%02X%02X%02X00%02X", tmp1[3], tmp1[4], tmp1[5], (MAC_sum_complement & 0xff));
						#endif
					}	
		#else				
					...
					if(0 == idx){
						...
						if(Mib_init ==1){
							apmib_get(MIB_HW_NIC0_ADDR, (void *)&tmp1); 
						#if defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
							sprintf((char *)pMib->wlan[idx][i].ssid, "RTK Audio Config AP_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
						#else
							memset(mysp,0,sizeof(mysp));
							MAC_sum_complement = -(tmp1[3] + tmp1[4] + tmp1[5]);
							sprintf(mysp, "@RSC-%02X%02X%02X00%02X", tmp1[3], tmp1[4],tmp1[5], (MAC_sum_complement & 0xff));
...
#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_APP_SIMPLE_CONFIG)	
				if(idx == 0)					
						pMib->wlan[idx][i].ScEnabled = 1;	
				else
						pMib->wlan[idx][i].ScEnabled = 0;
			#ifdef FOR_DUAL_BAND
				if (idx == 1)
					pMib->wlan[idx][i].ScEnabled = 1;
			#endif			
#endif		
#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_RTL_8881A_SELECTIVE)
				if(idx == 0) {
						pMib->wlan[idx][i].channelbonding = 2; //0 :20MHz 1:40Mhz 2:80MHz
						pMib->wlan[idx][i].wlanBand = BAND_5G_11AN|BAND_5G_11AC | BAND_11BG;
				}
#elif defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_RTL_8197F)
				if(idx == 0) {
						pMib->wlan[idx][i].channelbonding = 1; //0 :20MHz 1:40Mhz 2:80MHz
						pMib->wlan[idx][i].wlanBand = BAND_5G_11AN|BAND_5G_11AC;
				}
				if(idx == 1) {
						pMib->wlan[idx][i].channelbonding = 1; //0 :20MHz 1:40Mhz 2:80MHz
			...
			{	
#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_APP_SIMPLE_CONFIG)
				pMib->wlan[idx][i].wlanMode = AP_MODE;
				pMib->wlan[idx][i].ScEnabled = 0;	
#endif				

#ifdef CONFIG_RTL_92D_SUPPORT
				if(wlanPhyBandDef[idx] == PHYBAND_5G) {
				#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_RTL_8197F)
					pMib->wlan[idx][i].channel = 0;
				#else	
...
#endif
				{
					#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_RTL_8881A_SELECTIVE)&& defined(CONFIG_BAND_5G_ON_WLAN0)
						pMib->wlan[idx][i].channel = 0;
					#elif defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_RTL_8197F)
						pMib->wlan[idx][i].channel = 0;
					#else
	...
	#endif				
#endif
					pMib->wlan[idx][i].APRFBand = pMib->wlan[idx][i].wlanBand;
					pMib->wlan[idx][i].APRFChannel = 0;	
				}
		        else
#endif
        			{
						...
						pMib->wlan[idx][i].APRFBand = BAND_5G_11AN|BAND_5G_11AC;
						pMib->wlan[idx][i].APRFChannel = 0;
					#elif defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_RTL_8197F)
						pMib->wlan[idx][i].wlanBand = BAND_11BG | BAND_11N;
						pMib->wlan[idx][i].APRFBand = pMib->wlan[idx][i].wlanBand;
...
#if defined(CONFIG_APP_WIFIAUDIO) && !defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)			
				pMib->wlan[idx][i].encrypt = ENCRYPT_WPA2_MIXED;
				pMib->wlan[idx][i].wpa2Cipher = (unsigned char)WPA_CIPHER_AES;
				pMib->wlan[idx][i].wpaCipher = (unsigned char)WPA_CIPHER_AES; 
				sprintf(pMib->wlan[idx][i].wpaPSK, "%s", "12345678");
				pMib->wlan[idx][i].wscConfigured = 1;
				sprintf(pMib->wlan[idx][i].wscPsk, "%s", "12345678");
				pMib->wlan[idx][i].wscAuth = WSC_AUTH_WPA2PSKMIXED;
				pMib->wlan[idx][i].wscEnc = WSC_ENCRYPT_AES;
#endif			
...
#endif
#if defined(CONFIG_APP_WIFIAUDIO)
			pMib->wlan[idx][i].TxBeamforming = 0; 
#else
	...
	pMib->subnetMask[3] = 0;
#if defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
	pMib->AudioSinkConfigIpAddr[0] = 192;
	pMib->AudioSinkConfigIpAddr[1] = 168;
	pMib->AudioSinkConfigIpAddr[2] = 99;
	pMib->AudioSinkConfigIpAddr[3] = 254;
	pMib->AudioSinkConfigsubnetMask[0] = 255;
	pMib->AudioSinkConfigsubnetMask[1] = 255;
	pMib->AudioSinkConfigsubnetMask[2] = 255;
	pMib->AudioSinkConfigsubnetMask[3] = 0;
	pMib->AudioSinkConfigdhcpClientStart[0] = 192;
	pMib->AudioSinkConfigdhcpClientStart[1] = 168;
	pMib->AudioSinkConfigdhcpClientStart[2] = 99;
	pMib->AudioSinkConfigdhcpClientStart[3] = 200;

	pMib->AudioSinkConfigdhcpClientEnd[0] = 192;
	pMib->AudioSinkConfigdhcpClientEnd[1] = 168;
	pMib->AudioSinkConfigdhcpClientEnd[2] = 99;
	pMib->AudioSinkConfigdhcpClientEnd[3] = 210;
#endif
...
#if defined(CONFIG_RTL_8197F) &&defined(CONFIG_APP_WIFIAUDIO)
    pMib->ntpEnabled=1;
    pMib->daylightsaveEnabled=0;
    strcpy(pMib->ntpTimeZone,"-8 4");
    pMib->ntpServerIp1[0]=211;
    pMib->ntpServerIp1[1]=22;
    pMib->ntpServerIp1[2]=103;
    pMib->ntpServerIp1[3]=157;
    pMib->ntpServerIp2[0]=0;
    pMib->ntpServerIp2[1]=0;
    pMib->ntpServerIp2[2]=0;
    pMib->ntpServerIp2[3]=0;
...
#if defined(CONFIG_APP_WIFIAUDIO)
pMib->opMode = BRIDGE_MODE;
pMib->AudioSinkConfigMgmt = 0;
pMib->scrlogEnabled = 33;//syslog enable + avs log
flash_read(tmp1,HW_WLAN_SETTING_OFFSET + HW_SETTING_OFFSET,6);
#if defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
#if defined(CONFIG_RTL_8196E)
sprintf((char *)pMib->AudioSinkConfigDeviceName, "RTL8196E_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
#elif defined(CONFIG_RTL_8881A_SELECTIVE)
sprintf((char *)pMib->AudioSinkConfigDeviceName, "RTL8871AM_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
#elif defined(CONFIG_RTL_8197F)
sprintf((char *)pMib->AudioSinkConfigDeviceName, "RTL8197F_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
#else
sprintf((char *)pMib->AudioSinkConfigDeviceName, "%s", "RTKWiFiSP");
#endif
#endif

#if defined(CONFIG_APP_SIMPLE_CONFIG)
	#if defined(CONFIG_RTL_8196E)
	sprintf(pMib->scDeviceName, "RTL8196E_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
	#elif defined(CONFIG_RTL_8881A_SELECTIVE)
	sprintf(pMib->scDeviceName, "RTL8871AM_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
	#elif defined(CONFIG_RTL_8197F)
	sprintf(pMib->scDeviceName, "RTL8197F_%02X%02X%02X", tmp1[3], tmp1[4], tmp1[5]);
	#else
	sprintf((char *)pMib->AudioSinkConfigDeviceName, "%s", "RTKWiFiSP");
	#endif
pMib-> repeaterEnabled1 =1;
pMib-> repeaterEnabled2 =1;
#else
pMib-> repeaterEnabled1 =1;
pMib-> repeaterEnabled2 =0;
#endif
#ifdef MULTI_ROOM_SUPPORT
pMib->AudioSinkConfigSpeakerSet =2;
#endif
#endif
	data = (char *)pMib;
#ifdef _LITTLE_ENDIAN_
...
static int initWlan(char *ifname)
{
    ...
    apmib_get(MIB_WLAN_BSSID, (void*)buf1);
    //printf("%s - get bssid : %02x%02x%02x%02x%02x%02x\n", __func__, buf1[0], buf1[1], buf1[2], buf1[3], buf1[4], buf1[5]);
    memset(pmib->dot11StationConfigEntry.dot11DesiredBssid, 0, 6);
    unsigned char null_mac[6] = {0,0,0,0,0,0};
    if (memcmp(buf1, null_mac, 6)) {
        //printf("%s - set %02x%02x%02x%02x%02x%02x to bssid field\n", __func__, buf1[0], buf1[1], buf1[2], buf1[3], buf1[4], buf1[5]);
        memcpy(pmib->dot11StationConfigEntry.dot11DesiredBssid, buf1, 6);
    }
#endif
...
#if defined(CONFIG_APP_WIFIAUDIO) && (defined(CONFIG_RTL_8881A_SELECTIVE)||defined(CONFIG_RTL_8197F))
...
{
	char WifiPSKBuf[65];
	apmib_get( MIB_WLAN_WPA_PSK, (void *)WifiPSKBuf);
	if(encrypt > 0 && WifiPSKBuf[0] &&(strlen(WifiPSKBuf) ==5 || strlen(WifiPSKBuf) ==10)){
		apmib_get(MIB_WLAN_WEP64_KEY1, (void *)buf1);
		memcpy(&(pmib->dot11DefaultKeysTable.keytype[0]), buf1, 5);
		apmib_get(MIB_WLAN_WEP64_KEY2, (void *)buf1);
		memcpy(&(pmib->dot11DefaultKeysTable.keytype[1]), buf1, 5);
	...
	if(encrypt > 0 && WifiPSKBuf[0] &&(strlen(WifiPSKBuf) ==13 || strlen(WifiPSKBuf) ==26)){
		apmib_get(MIB_WLAN_WEP128_KEY1, (void *)buf1);
		memcpy(&(pmib->dot11DefaultKeysTable.keytype[0]), buf1, 13);
		apmib_get(MIB_WLAN_WEP128_KEY2, (void *)buf1);
		memcpy(&(pmib->dot11DefaultKeysTable.keytype[1]), buf1, 13);
