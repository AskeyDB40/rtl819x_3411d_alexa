...
extern void StartInternetRadioRenderer(void);
extern void CleanNetRadioRenderer(void);
extern void CleanDlnaRenderer(void);
extern void StartDlnaRenderer(void);
extern void Wait_DLNA_Started(void);
#endif
extern void start_ntp(void);
...
#if defined(CONFIG_AUDIO_SINK_SUPPORT) 
void AudioConfigCreate_bound_script(char *script_path, char *iface)
{
...
void set_lan_dhcpc(char *iface)
{
	...
	int intValue=0;
#endif
#if defined(CONFIG_AUDIO_SINK_SUPPORT) && defined(CONFIG_APP_WIFIAUDIO)
	sprintf(cmdBuff,"/usr/share/udhcpc/%s.bound", iface);
	AudioConfigCreate_bound_script(cmdBuff, iface);
#endif	
...
void lan_connect(char *interface, char *option)
{
	...
	if(lan_type==DHCP_CLIENT && !strcmp(interface, "br0") && mgmt == 0){
		CleanDlnaRenderer();
		...
		system("/bin/alexa fmt_conv48_card1_ch1 format_convert16_ch1 &");
		#else
/* Askey added start, Cooper, 12/04/2017 */
/* Added for date sync when Alexa set timmer. */
		system("ntpclient  -s -i 10 -h 216.239.35.4");
/* Askey added end, Cooper, 12/04/2017 */
		system("/bin/alexa format_convert16_ch1 format_convert16_ch1 &");
		#endif
	#if defined(CONFIG_DLNA_RENDERER_SUPPORT)	
	system("rm -f /var/rygel-1.0/playbin_loadded");
	StartDlnaRenderer();
	Wait_DLNA_Started();	
	#endif
	#if defined(CONFIG_INTERNET_RADIO_SUPPORT)
			StartInternetRadioRenderer();
	#endif
	}
#endif	
