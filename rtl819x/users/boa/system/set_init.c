...
#endif
#define DEFAULT_RENDERER_PID_FILENAME		("/var/run/renderer-pid")
...
void CleanNetRadioRenderer(void)
{
	unlink("/var/run/netradio.pid");
	system("killall netRadioServer 2> /dev/null");
}
void CleanDlnaRenderer(void)
{
	char cmdBuff[512]={0};
	int pid = getNumberFromFile(DEFAULT_RENDERER_PID_FILENAME);
	
	if(pid>0){
		sprintf(cmdBuff, "kill -15 %d", pid);
		system(cmdBuff);
		system("killall rygel 2> /dev/null");
		unlink(DEFAULT_RENDERER_PID_FILENAME);
	}
}

void StartDlnaRenderer(void)
{
	char cmdBuff[512]={0};
	int pid = getNumberFromFile(DEFAULT_RENDERER_PID_FILENAME);
	char destnet_iface[32]={0};
#if defined(CONFIG_APP_WIFIAUDIO) 	&& defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
	int isACConfigured=0;
	int isSCSaveProfiled=0;
	int mgmt=0;
#endif
#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
		apmib_get( MIB_AUDIO_SINK_CONFIG_MGMT, (void *)&mgmt);
		if(SetWlan_idx("wlan0-vxd")){	
			apmib_get( MIB_WLAN_AUDIO_SINK_CONFIG_CONFIGURED, (void *)&isACConfigured); 
			apmib_get( MIB_WLAN_SC_SAVE_PROFILE, (void *)&isSCSaveProfiled);
		}
#endif	
#if defined(CONFIG_APP_WIFIAUDIO) && defined(CONFIG_AUDIO_SINK_CONFIG_SUPPORT)
		if(isACConfigured == 0 && isSCSaveProfiled == 0 && mgmt ==1){
			sprintf(destnet_iface, "%s", "br1");
		}else
			sprintf(destnet_iface, "%s", "br0");
#else
			sprintf(destnet_iface, "%s", "br0");
#endif
	...
	if(isFileExist("/usr/bin/rygel") !=0){
		printf("Start DMR\n");
			RunSystemCmd(NULL_FILE, "route", "del", "-net", "239.255.255.250", "netmask", "255.255.255.255", destnet_iface, NULL_STR); 
			RunSystemCmd(NULL_FILE, "route", "add", "-net", "239.255.255.250", "netmask", "255.255.255.255",destnet_iface, NULL_STR); 
			sprintf(cmdBuff, "/usr/bin/rygel -g 5 -n %s &", destnet_iface);
		//sprintf(cmdBuff, "/usr/bin/rygel -n %s &", destnet_iface);
		system(cmdBuff);
	}
}

void StartInternetRadioRenderer(void)
{
	if(isFileExist("/bin/netRadioServer") !=0){
		printf("Start internet radio\n");
		system("netRadioServer &");
	}
}
void Wait_DLNA_Started(void)
{
	if(isFileExist("/usr/bin/rygel") !=0){
		for(;;) {
			if(isFileExist("/var/rygel-1.0/playbin_loadded")){
				break;
			}else 
				usleep(10000);
		}
...
#endif //#if defined(CONFIG_APP_WIFIAUDIO)
...
#endif
#if defined(CONFIG_APP_WIFIAUDIO)
	system("killall alexa > /dev/null 2>&1");
	system("killall auth_web > /dev/null 2>&1");
	CleanDlnaRenderer();
	CleanNetRadioRenderer();
#endif
}
...
#if defined(HOME_GATEWAY) && !defined(CONFIG_RTL_AP_PACKAGE) 
...
#endif
#if defined(CONFIG_APP_WIFIAUDIO)
	if(isFileExist("/var/system/boa_started")==0){
			system("boa");
			system("echo 1 > /var/system/boa_started");
	}
#endif
	return 0;
}
