...
//printf("\r\n buffer[%s],__[%s-%u]\r\n",buffer,__FILE__,__LINE__);			
	...
	else if(!strcmp(argv[0],("wfaudio_jscomment_end")))
	{
		req_format_write(wp, "");
		return 0;
	}
	else if(!strcmp(argv[0],("wfaudio_jscomment_start")))
	{
		req_format_write(wp, "");
		return 0;
	}
	else if(!strcmp(argv[0],("wfaudio_comment_start")))
	{
		req_format_write(wp, "");
		return 0;
	}
	else if(!strcmp(argv[0],("wfaudio_comment_end")))
	{
		req_format_write(wp, "");
		return 0;
	}
...
//	for ap package of new ui, start
		...
		||  !strcmp(name, "show_mgt_dos")  ) {	
#if defined(CONFIG_RTL_AP_PACKAGE)	
			return req_format_write(wp,"0");
#else	
			return req_format_write(wp,"1");
#endif
	} else if (!strcmp(name, "show_mgt_ntp")) {
			return req_format_write(wp,"1");
	}
