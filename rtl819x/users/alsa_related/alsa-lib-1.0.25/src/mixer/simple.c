...
int set_playback_mixer_control_wrapper(const char *sound_card, const char *ControlName, double InValue)
{
	int err=0;
	long min, max, target;
	double vol_max=0.0;
	...
	if ((err = snd_mixer_open(&mixer_handle, 0)) < 0) {
		fprintf(stderr, "Mixer %s open error: %s\n", sound_card, snd_strerror(err));
		return err;
	}
	
	if ((err = snd_mixer_attach(mixer_handle, sound_card)) < 0) {
		fprintf(stderr, "%s %d Mixer attach %s error: %s\n", __FUNCTION__, __LINE__, sound_card, snd_strerror(err));
	...
	//fprintf(stderr, "SET ALSA@ sound_card:%s ControlName:%s  =>%f\n",sound_card,ControlName,InValue);
	...
	for (elem = snd_mixer_first_elem(mixer_handle); elem; elem = snd_mixer_elem_next(elem)) {
		...
		//fprintf(stderr, "ALSA@ snd_mixer_selem_id_get_name:%s\n",controlName);
		if (!strcmp(controlName, ControlName) && snd_mixer_selem_has_playback_volume(elem)) {
			
			snd_mixer_selem_get_playback_volume_range(elem, &min, &max);
			vol_current =(double)InValue;
			vol_max = (double)max;
			target_vol = (vol_max*vol_current)/1024.0;
			target_vol = round(target_vol);
			target = (long)target_vol;
			//fprintf(stderr, "target vol=%f, min=%ld, max=%ld, vol_current=%ld\n",target_vol, min, max, vol_current);
			if(target >= min && target <= max)
				err=snd_mixer_selem_set_playback_volume_all(elem, target);
				else
					fprintf(stderr, "ALSA: set_playback_mixer_control_wrapper failed: %d\n",err);
			if( err < 0 ) {
				fprintf(stderr, "ALSA: set_playback_mixer_control_wrapper failed: %d\n",err);
			} 
			if (snd_mixer_selem_has_playback_switch(elem)) {
				if(target == 0){
					if( (err=snd_mixer_selem_set_playback_switch_all(elem, 0)) < 0 ) {
						fprintf(stderr, "ALSA: set_playback_mixer_control_wrapper failed: %d\n",err);
					}	 
				}else{
...
int get_playback_mixer_control_wrapper(const char *sound_card, const char *ControlName, double *OutValue)
{
	...
	*OutValue = 0.0;
	...
	//fprintf(stderr, "GET ALSA@ sound_card:%s ControlName:%s\n",sound_card,ControlName);
	...
	for (elem = snd_mixer_first_elem(mixer_handle); elem; elem = snd_mixer_elem_next(elem)) {
		...
		if (!strcmp(controlName, ControlName) && snd_mixer_selem_has_playback_volume(elem)) {
			...
			snd_mixer_selem_get_playback_volume_range(elem, &min, &max);
			vol_max = (double)max;
			err=snd_mixer_selem_get_playback_volume(elem, 0, &current_sound_vol);
			if( err < 0 ) {
				fprintf(stderr, "ALSA: snd_mixer_selem_get_playback_volume failed: %d\n",err);
			}else{ 
				vol_current =(double)current_sound_vol;
				
				target_vol = (vol_current*1024.0)/vol_max;
				//fprintf(stderr, "target vol=%f, min=%ld, max=%ld\n",target_vol, min, max);
				*OutValue = target_vol;
			}
			break;
		}else{
