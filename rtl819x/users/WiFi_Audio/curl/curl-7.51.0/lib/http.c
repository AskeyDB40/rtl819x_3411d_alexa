                                 ...
                                 Curl_send_buffer *req_buffer)
{
  ...
  struct Curl_easy *data = conn->data;
  int i;
	int Current_httpversion = conn->httpversion; 
    ...
    while(headers) {
      ...
      if(ptr) {
        ...
        if(*ptr) {
      	...
      	if (Current_httpversion == 20) {
      		if (headers->data)
      			ptr1 = strstr(headers->data, "=");
      		else {
      			printf("%s %d headers->data NULL\n", __FUNCTION__, __LINE__);
      			ptr1 =NULL;
      		}
      		if (ptr1) {
      			 CURLcode check_result;
      			 check_result = Curl_add_bufferf(req_buffer, "%s\r\n", headers->data);
      			  if(check_result)
	                		return check_result;
      		}else {
		ptr = strchr(headers->data, ';');
