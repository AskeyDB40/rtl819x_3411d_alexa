                          ...
                          const void *mem, size_t len, CURLcode *err)
{
  ...
  char *check_str=NULL;
	char str1[1576];
	char str2[1024];
	char *check_header_line_end=NULL;
	char *check_header_line=NULL;	
	char *check_header_line_next=NULL;
	int header_line_len=0;
	char *check_method=NULL;
	char *check_path=NULL;
	char str_method[128];
	char str_path[128];
	char *check_auth=NULL;
	char *check_content_type=NULL;
	char str_auth[1576];
	char str_content_type[256];
	char str_auth_value[1576];
	char str_content_type_value[256];
	char *check_hdbuf = (char*)mem;
  (void)sockindex;
//printf("~~len= %d , hdbuf:\n", len);
//printf("%s\n", hdbuf);
check_header_line_end = strstr(hdbuf, "\r\n\r\n");
if (check_header_line_end){
	header_line_len = check_header_line_end-hdbuf;
	//printf("~~header len= %d\n", header_line_len);
}
...
do {  
	 check_header_line=strstr(check_hdbuf, "\r\n");
	 if (check_header_line) {
	 	snprintf( str1, (check_header_line-check_hdbuf)+1, "%s", check_hdbuf);
	 	//printf("%s\n", str1);
	 	check_hdbuf = check_header_line+strlen("\r\n");
	 	nheader++;
	}
}while(check_header_line != check_header_line_end);
//printf("nheader=%d\n", nheader);
	...
	*err = CURLE_OUT_OF_MEMORY;
	...
	do {  
		 ...
		 if (check_header_line) {
		 	...
		 	//printf("%s\n", str1);
		 	check_hdbuf = check_header_line+strlen("\r\n");
		 	if (str1[0] == ':' || strchr(str1, '=')) {
		 	if (str1[0] == ':') {
		 		if (i==0) {
					check_method = strstr(str1, "=");
					sprintf(str_method, "%s", check_method+2);
					nva[i].name = (unsigned char *)":method";
					nva[i].namelen = strlen((char *)nva[i].name);
					nva[i].value = (unsigned char *)str_method;
					nva[i].valuelen = (size_t)(strlen(str_method));
					...
					nva[i].name = (unsigned char *)":scheme";
					nva[i].namelen = strlen((char *)nva[i].name);
					nva[i].value = (unsigned char *)"https";
					nva[i].valuelen = strlen((char *)nva[i].value);
					nva[i].flags = NGHTTP2_NV_FLAG_NONE;
					if(HEADER_OVERFLOW(nva[i])) {
						failf(conn->data, "Failed sending HTTP request: Header overflow");
						goto fail;
					...
					check_path = strstr(str1, "=");
					sprintf(str_path, "%s", check_path+2);
					nva[i].name = (unsigned char *)":path";
					nva[i].namelen = strlen((char *)nva[i].name);
					nva[i].value = (unsigned char *)str_path;
					nva[i].valuelen = (size_t)(strlen(str_path));
					nva[i].flags = NGHTTP2_NV_FLAG_NONE;
					if(HEADER_OVERFLOW(nva[i])) {
						failf(conn->data, "Failed sending HTTP request: Header overflow");
						goto fail;
		 		...
		 		if (i==3) {
		 		check_str= strstr(str1, "=");
		 		check_str = check_str-1;
		 		snprintf(str_auth, (check_str-str1)+1, "%s",str1);
				nva[i].name = (unsigned char *)str_auth;
				nva[i].namelen = strlen((char *)nva[i].name);
				
				check_str= strstr(str1, "=");
		 		check_str = check_str+2;
				sprintf(str_auth_value, "%s", check_str);
				nva[i].value = (unsigned char *)str_auth_value;
				nva[i].valuelen = (size_t)(strlen(str_auth_value));
				nva[i].flags = NGHTTP2_NV_FLAG_NONE;
				if(HEADER_OVERFLOW(nva[i])) {
					failf(conn->data, "Failed sending HTTP request: Header overflow");
					goto fail;
				...
				i++;
					...
					check_str= strstr(str1, "=");
			 		check_str = check_str-1;
			 		snprintf(str_content_type, (check_str-str1)+1, "%s",str1);
					nva[i].name = (unsigned char *)str_content_type;
					nva[i].namelen = strlen((char *)nva[i].name);
					
					check_str= strstr(str1, "=");
			 		check_str = check_str+2;
					sprintf(str_content_type_value, "%s", check_str);
					nva[i].value = (unsigned char *)str_content_type_value;
					nva[i].valuelen = (size_t)(strlen(str_content_type_value));
					nva[i].flags = NGHTTP2_NV_FLAG_NONE;
					if(HEADER_OVERFLOW(nva[i])) {
						failf(conn->data, "Failed sending HTTP request: Header overflow");
						goto fail;
