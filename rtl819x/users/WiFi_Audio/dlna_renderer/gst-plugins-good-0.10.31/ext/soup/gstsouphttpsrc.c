...
g_object_class_install_property (gobject_class, PROP_SEEK_MODE,          
           g_param_spec_uint ("seek-mode", "seek-mode",
          "Value in Range seek or not", 0,3, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_TIMEOUT:
      ...
      break;
      case PROP_SEEK_MODE:
      	src->seek_mode = g_value_get_uint (value);
      break;
    case PROP_EXTRA_HEADERS:{
	const GValue *tmp_value=NULL;
	gboolean ret;
	unsigned int isAppend=0;
	const GstStructure *s = gst_value_get_structure (value);
	gchar *c_str=NULL;
	if (src->extra_headers) {
		tmp_value = gst_structure_get_value (s, "TimeSeekRange.dlna.org");
		if (tmp_value != NULL && G_VALUE_TYPE (tmp_value) == G_TYPE_STRING) {
			ret = gst_structure_get (s, "TimeSeekRange.dlna.org", G_TYPE_STRING, &c_str, NULL);
			if (ret) {
				//g_message("%s %d String:%s, len=%d", __FUNCTION__, __LINE__, c_str, strlen(c_str));
				gst_structure_set(src->extra_headers, "TimeSeekRange.dlna.org", G_TYPE_STRING, c_str, NULL);
				isAppend = 1;
				g_free(c_str);
			}else
				g_message("%s %d get String is Null", __FUNCTION__, __LINE__);
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_TIMEOUT:
      ...
      break;
       case PROP_SEEK_MODE:
       	g_value_set_uint (value, src->seek_mode);
       	break;
    case PROP_EXTRA_HEADERS:
...
gst_soup_http_src_add_range_header (GstSoupHTTPSrc * src, guint64 offset)
{
 ...
 //seek_mode means add "Range" in http header
 //g_message("%s %d seek_mode=%d", __FUNCTION__, __LINE__, src->seek_mode);
  if (offset && src->seek_mode == 1) { 
...
gst_soup_http_src_got_headers_cb (SoupMessage * msg, GstSoupHTTPSrc * src)
{
  ...
  if (soup_message_headers_get_encoding (msg->response_headers) ==
   ...
   // if (!src->have_size || (src->content_size != newsize)) {
    if ((!src->have_size || (src->content_size != newsize))&&(src->seek_mode != 2 || src->content_size==0)) {
      ...
      gst_element_post_message (GST_ELEMENT (src),
    	...
    	if (src->seek_mode == 2) {
    		 if ((value = soup_message_headers_get_one (msg->response_headers,"TimeSeekRange.dlna.org")) != NULL) {
    		 	g_message("%s %d seek range:%s", __FUNCTION__, __LINE__, value);
    		}
    	}
    	if (src->seek_mode == 1) {
    		 if ((value = soup_message_headers_get_one (msg->response_headers,"Content-Range")) != NULL) {
    		 	g_message("%s %d seek range:%s", __FUNCTION__, __LINE__, value);
    		}
    	}
    }
  ...
  if (src->ret == GST_FLOW_CUSTOM_ERROR &&
      src->read_position && src->seek_mode == 1 && msg->status_code != SOUP_STATUS_PARTIAL_CONTENT) {
    src->seekable = FALSE;
  ...
  if (src->session_io_status == GST_SOUP_HTTP_SRC_SESSION_IO_STATUS_CANCELLED) {
    ...
    GST_ELEMENT_ERROR (src, RESOURCE, NOT_FOUND,
        ...
        if(msg->reason_phrase)
				g_message("%s, %d reason=%s", __FUNCTION__, __LINE__,msg->reason_phrase );
...
guint8 *data;
gint byte_endianness=G_BIG_ENDIAN;
GstStructure *s=NULL;
gboolean ret = TRUE;
  if (G_UNLIKELY (msg != src->msg)) {
  ...
  *src->outbuf = GST_BUFFER_CAST (soup_buffer_get_owner (chunk));
	data = GST_BUFFER_DATA (*src->outbuf);
	if (src->raw_audio_endian == 1) {
		if (*data == 0x52 && *(data+1) == 0x49 && *(data+2)==0x46 && *(data+3) == 0x46 ) { //little endian
			if (src->src_caps) {
				const GValue *value=NULL;
				s = gst_caps_get_structure (src->src_caps, 0);
				value = gst_structure_get_value (s, "endianness");
				if (G_VALUE_TYPE (value) == G_TYPE_INT) {
			    		ret = gst_structure_get (s, "endianness", G_TYPE_INT, &byte_endianness, NULL);
			    		if (byte_endianness != G_LITTLE_ENDIAN) {
			    			g_message("update endian to little endian!");
			    			  src->src_caps =  gst_caps_make_writable (src->src_caps);
			    			  s = gst_caps_get_structure (src->src_caps, 0);
			    			  gst_structure_set (s, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, NULL);
			    		}
				}
			}
		}
...
gst_soup_http_src_parse_status (SoupMessage * msg, GstSoupHTTPSrc * src)
{
  if (SOUP_STATUS_IS_TRANSPORT_ERROR (msg->status_code)) {
    ...
    src->ret = GST_FLOW_ERROR;
  	...
  	if (msg->status_code == 200 && src->seek_mode != 1) {
  		 src->ret = GST_FLOW_OK;
  	}
	}
}
...
#if 0   //this field should set by application self
...
gst_soup_http_src_create (GstPushSrc * psrc, GstBuffer ** outbuf)
{
  ...
  do {
    ...
    if (src->retry) {
      ...
      g_message("%s %d Do Reconnecting........", __FUNCTION__, __LINE__);
	if (src->iradio_mode ==1) {
		system("echo 1 > /var/httpsrc_reconnect");
	}
      if (!gst_soup_http_src_build_message (src))
        return GST_FLOW_ERROR;
      src->retry = FALSE;
