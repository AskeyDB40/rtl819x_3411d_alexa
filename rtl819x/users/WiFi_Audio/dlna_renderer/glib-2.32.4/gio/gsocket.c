...
if(socket->priv->type == G_SOCKET_TYPE_STREAM &&(socket->priv->family ==GLIB_SYSDEF_AF_INET))    
{
	int flag = 1;
	int err;
	...
	int ret=0;
	int errsv;
	err = setsockopt(new_socket->priv->fd, IPPROTO_TCP, TCP_NODELAY, (void *)&flag, sizeof(flag));
	if (err == -1) {
		errsv = get_socket_errno ();
		g_warning("(%s)Couldn't setsockopt(TCP_NODELAY) %s \n", __FUNCTION__,socket_strerror (errsv));
	}
	
	opt = priority << 5;
	ret = setsockopt(new_socket->priv->fd, SOL_IP, IP_TOS, &opt, sizeof(opt));
	if (ret < 0){
		errsv = get_socket_errno ();
		g_warning("(%s)Couldn't setsockopt(IP_TOS) %s \n", __FUNCTION__,socket_strerror (errsv));
	}
	opt = 6;
	ret = setsockopt(new_socket->priv->fd, SOL_SOCKET, SO_PRIORITY, &opt, sizeof(opt));
	if (ret < 0){
		errsv = get_socket_errno ();
		g_warning("(%s)Couldn't setsockopt(SO_PRIORITY) %s \n", __FUNCTION__,socket_strerror (errsv));
	}
...
int g_socket_setopt_tcp (GSocket         *g_socket)
{
	int flag = 1;
	int err;
	unsigned int priority=0;
	...
	int ret=0;
	int errsv;
		if(g_socket->priv->type == G_SOCKET_TYPE_STREAM &&(g_socket->priv->family ==GLIB_SYSDEF_AF_INET))
		{
	
		err = setsockopt(g_socket->priv->fd, IPPROTO_TCP, TCP_NODELAY, (void *)&flag, sizeof(flag));
		if (err == -1) {
			errsv = get_socket_errno ();
			g_warning("(%s)Couldn't setsockopt(TCP_NODELAY) %s \n", __FUNCTION__,socket_strerror (errsv));
		}
		...
		opt = priority << 5;
		ret = setsockopt(g_socket->priv->fd, SOL_IP, IP_TOS, &opt, sizeof(opt));
		if (ret < 0){
			errsv = get_socket_errno ();
			g_warning("(%s)Couldn't setsockopt(IP_TOS) %s \n", __FUNCTION__,socket_strerror (errsv));
		}
		opt = 6;
		ret = setsockopt(g_socket->priv->fd, SOL_SOCKET, SO_PRIORITY, &opt, sizeof(opt));
		if (ret < 0){
			errsv = get_socket_errno ();
			g_warning("(%s)Couldn't setsockopt(SO_PRIORITY) %s \n", __FUNCTION__,socket_strerror (errsv));
		}
...
if(socket->priv->type == G_SOCKET_TYPE_STREAM &&(socket->priv->family ==GLIB_SYSDEF_AF_INET))
{
	...
	int ret=0;
	int errsv_;
	err = setsockopt(socket->priv->fd, IPPROTO_TCP, TCP_NODELAY, (void *)&flag, sizeof(flag));
	if (err == -1) {
		errsv_ = get_socket_errno ();
		g_warning("(%s)Couldn't setsockopt(TCP_NODELAY) %s \n", __FUNCTION__,socket_strerror (errsv_));
	}

	opt = priority << 5;
	ret = setsockopt(socket->priv->fd, SOL_IP, IP_TOS, &opt, sizeof(opt));
	if (ret < 0){
		errsv_ = get_socket_errno ();
		g_warning("(%s)Couldn't setsockopt(IP_TOS) %s \n", __FUNCTION__,socket_strerror (errsv_));
	}
	opt = 6;
	ret = setsockopt(socket->priv->fd, SOL_SOCKET, SO_PRIORITY, &opt, sizeof(opt));
	if (ret < 0){
		errsv_ = get_socket_errno ();
		g_warning("(%s)Couldn't setsockopt(SO_PRIORITY) %s \n", __FUNCTION__,socket_strerror (errsv_));
	}
