...
unsigned int CheckUserAgent(const char *ClientUserAgent);
...
typedef struct {
        ...
        gboolean      to_delete;
        char	client_addr[32];
        char	current_served_client_addr[32];
} SubscriptionData;
...
unsigned int CheckUserAgent(const char *ClientUserAgent)
{
	...
	char  *str_subToken=NULL;
            ...
            snprintf(tmpUsrAgent, 256, "%s", ClientUserAgent);
            if(tmpUsrAgent[0]){
			str_Token = strstr(tmpUsrAgent, "Windows");
			if(str_Token){
				return MS_WINDOWS;
			}else {
				str_Token = strstr(tmpUsrAgent, "Galaxy");
				if(str_Token){
					//Galaxy
					return GALAXY;
				}else{
					str_Token = strstr(tmpUsrAgent, "Twonky");
					if(str_Token){
						//TWONKY
						return TWONKY;
					}else{
						str_Token = strstr(tmpUsrAgent, "BubbleUPnP");
						if(str_Token){
							//BubbleUPnP
							return BUBBLEUPNP;
						}else{
							str_Token = strstr(tmpUsrAgent, "Portable");
							if(str_Token){
								//QQ Music
								return PORTABLE;
							}else{
...
gupnp_service_init (GUPnPService *service)
{
        ...
        service->priv->GenerateTransition = 0;
                        ...
                        gpointer           user_data)
{
        ...
        *action_name = '\0';
        ...
        //sprintf(action->client_addr, "%s", soup_client_context_get_host (soup_client)); 
		if(soup_client_context_get_host_ip (soup_client, action->client_addr) ==0){
			action->client_addr[0]='\0';
		}
        for (node = action->node->children; node; node = node->next)
	...
	Client_UserAgent = soup_message_headers_get_one (msg->request_headers,
                                                         "User-Agent");                                                         
	if(Client_UserAgent){
		action->client_UserAgent = CheckUserAgent(Client_UserAgent);
	}
...
send_initial_state (SubscriptionData *data)
{
        GQueue *queue;
        //char *mem;
        struct _NotifyData_Property mem;
        GList *l;
        ...
        mem.PropertyData = create_property_set (queue);
        mem.isIn_notify_transition = 0;
        notify_subscriber (data->sid, data, &mem);

        /* Cleanup */
        g_queue_free (queue);

        g_free (mem.PropertyData);
}
...
subscribe (GUPnPService *service,
           SoupMessage  *msg,
           const char   *callback, char *ClientIP)
{
        SubscriptionData *data;
        char *start, *end, *uri;
        ...
        if(ClientIP[0] != '\0'){
		sprintf(data->client_addr,"%s", ClientIP);
		}else
			data->client_addr[0] = '\0';
			
		memset(data->current_served_client_addr, '\0', 32);
...
//        sprintf(ClientIPAddress, "%s", soup_client_context_get_host (soup_client)); 
		if(soup_client_context_get_host_ip (soup_client, ClientIPAddress)==0){
			ClientIPAddress[0]='\0';
		}
        ...
        if (strcmp (msg->method, GENA_METHOD_SUBSCRIBE) == 0) {
                if (callback) {
                        if (sid) {
                                ...
                                subscribe (service, msg, callback, ClientIPAddress);
                   ...
                   gpointer user_data)
{
        ...
        struct _NotifyData_Property *property_set;
        char *tmp;
        SoupMessage *msg;
        SoupSession *session;
	...
	if(data->current_served_client_addr[0] != '\0')
	{
		if(property_set->isIn_notify_transition ==1){ 
			if(!strcmp(data->client_addr, data->current_served_client_addr)){
						return;
			}
			
		}else if(property_set->isIn_notify_transition ==0){
			if(strcmp(data->client_addr, data->current_served_client_addr)){
						return;
			}
		}
	...
	soup_message_headers_append (msg->request_headers, "Connection", "close");
		...
		soup_message_set_request (msg,
                                  "text/xml; charset=\"utf-8\"",
                                  SOUP_MEMORY_TAKE,
                                  g_strdup (property_set->PropertyData),
                                  strlen (property_set->PropertyData));
#endif
        /* Queue */
...
void Set_GenerateTransition(GUPnPService *service, int State)
{
	service->priv->GenerateTransition=State;
}

void update_SubscriptionsCurrentServed(gpointer key, gpointer value, gpointer user_data)
{
		char *ValidIP=user_data;
        	SubscriptionData *data;
		data = value;
		sprintf(data->current_served_client_addr, "%s",ValidIP); 
}

void update_CurrentServed(GUPnPService *service, char *CurrentServedIP)
{
        g_hash_table_foreach (service->priv->subscriptions, update_SubscriptionsCurrentServed,CurrentServedIP);
...
flush_notifications (GUPnPService *service)
{
		...
		struct _NotifyData_Property mem;
        /* Create property set */
		mem.PropertyData = create_property_set (service->priv->notify_queue);
		mem.isIn_notify_transition=service->priv->GenerateTransition;
		service->priv->GenerateTransition = 0;
        /* And send it off */
