...
#define AUDIO_SRC_CAPS_FILE "/var/gstreamer-0.10/input_gstcaps"
struct _pcmrate_
{
  int rate;
  int isAvailavle;
};
extern struct _pcmrate_ _PCM_RATE_[];
extern int num_rate;
extern int alsasink_isFileExist(const char *file_name);
...
GValue rates_arr = { 0, };
	GValue tmp = { 0, };
	int sample_rate=0, byte_endianness,bit_width,bit_depth;
  ...
  GST_DEBUG_OBJECT (obj, "Max. rate = %u (%d)", max_rate, max);
#if (defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)  
	if (access(AUDIO_SRC_CAPS_FILE,R_OK) == 0) {	
		fp= fopen(AUDIO_SRC_CAPS_FILE, "r");
		if (!fp) {
        		g_message("can not open:%s\n", "/var/gstreamer-0.10/input_gstcaps");
			goto min_rate_err;
   		}
	...
	g_value_init (&rates_arr, GST_TYPE_LIST);
  	...
  	for (i = 0; i < num_rate; i++) {
  		if(_PCM_RATE_[i].isAvailavle ==1){
			g_value_set_int (&tmp, _PCM_RATE_[i].rate);
    ...
    if (min_rate == max_rate) {
      ...
      gst_structure_set (s, "rate", GST_TYPE_INT_RANGE,
	    ...
	    if(sample_rate > 0  && sample_rate <= 192000){
	    		gst_structure_set (s, "rate", G_TYPE_INT, sample_rate, NULL);
	  	}else
...
struct _pcmformats_ pcmformats_be[] = {
	{ 8, 8, SND_PCM_FORMAT_S8, SND_PCM_FORMAT_U8}, 
	{16, 16, SND_PCM_FORMAT_S16_BE, SND_PCM_FORMAT_U16_BE}, 
	{32, 24, SND_PCM_FORMAT_S24_BE, SND_PCM_FORMAT_U24_BE}, 
	{24, 24, SND_PCM_FORMAT_S24_3BE, SND_PCM_FORMAT_U24_3BE}, 
	{32, 32, SND_PCM_FORMAT_S32_BE, SND_PCM_FORMAT_U32_BE}
};
   
struct _pcmformats_ pcmformats_le[] = {
	{ 8, 8, SND_PCM_FORMAT_S8, SND_PCM_FORMAT_U8}, 
	{16, 16, SND_PCM_FORMAT_S16_LE, SND_PCM_FORMAT_U16_LE}, 
	{32, 24, SND_PCM_FORMAT_S24_LE, SND_PCM_FORMAT_U24_LE}, 
	{24, 24, SND_PCM_FORMAT_S24_3LE, SND_PCM_FORMAT_U24_3LE}, 
	{32, 32, SND_PCM_FORMAT_S32_LE, SND_PCM_FORMAT_U32_LE}
};

static GstCaps *
gst_alsa_detect_formats_be (GstObject * obj, snd_pcm_hw_params_t * hw_params,
    GstCaps * in_caps)
{
  ...
  for (i = 0; i < gst_caps_get_size (in_caps); ++i) {
    ...
    for (w = 0; w < G_N_ELEMENTS (pcmformats_be); w++)
      if (pcmformats_be[w].width == width && pcmformats_be[w].depth == depth)
        break;
    if (w == G_N_ELEMENTS (pcmformats_be))
      continue;                 /* Unknown format */

    if (snd_pcm_format_mask_test (mask, pcmformats_be[w].sformat) &&
        snd_pcm_format_mask_test (mask, pcmformats_be[w].uformat)) {
      /* template contains { true, false } or just one, leave it as it is */
      scopy = gst_structure_copy (s);
    } else if (snd_pcm_format_mask_test (mask, pcmformats_be[w].sformat)) {
      scopy = gst_structure_copy (s);
      gst_structure_set (scopy, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
    } else if (snd_pcm_format_mask_test (mask, pcmformats_be[w].uformat)) {
      scopy = gst_structure_copy (s);
    ...
    if (scopy) {
      if (width > 8) {
        ...
        gst_structure_set (scopy, "endianness", G_TYPE_INT, G_BIG_ENDIAN, NULL);
      }
...
static GstCaps *
gst_alsa_detect_formats_le (GstObject * obj, snd_pcm_hw_params_t * hw_params,
    GstCaps * in_caps)
{
  snd_pcm_format_mask_t *mask;
  GstStructure *s;
  ...
  for (i = 0; i < gst_caps_get_size (in_caps); ++i) {
    ...
    if (width == 0 || (width % 8) != 0)
      continue;                 /* Only full byte widths are valid */
    for (w = 0; w < G_N_ELEMENTS (pcmformats_le); w++)
      if (pcmformats_le[w].width == width && pcmformats_le[w].depth == depth)
        break;
    if (w == G_N_ELEMENTS (pcmformats_le))
      continue;                 /* Unknown format */

    if (snd_pcm_format_mask_test (mask, pcmformats_le[w].sformat) &&
        snd_pcm_format_mask_test (mask, pcmformats_le[w].uformat)) {
      /* template contains { true, false } or just one, leave it as it is */
      scopy = gst_structure_copy (s);
    } else if (snd_pcm_format_mask_test (mask, pcmformats_le[w].sformat)) {
      scopy = gst_structure_copy (s);
      gst_structure_set (scopy, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
    } else if (snd_pcm_format_mask_test (mask, pcmformats_le[w].uformat)) {
      scopy = gst_structure_copy (s);
      gst_structure_set (scopy, "signed", G_TYPE_BOOLEAN, FALSE, NULL);
    } else {
      scopy = NULL;
    ...
    if (scopy) {
      if (width > 8) {
        gst_structure_set (scopy, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, NULL);
...
#if (defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)  
  int sample_rate=0, byte_endianness,bit_width,bit_depth;
	int caps_signed, caps_channels=0;
	FILE *fp;
#endif
...
#if (defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)    
	if(caps_channels > 0 && caps_channels <=2){
		c_min = caps_channels;
		c_max = caps_channels;
	}
#endif	
...
GstCaps *gst_alsa_probe_supported_formats (GstObject * obj, snd_pcm_t * handle,  const GstCaps * template_caps, int endianness)
#else
GstCaps *gst_alsa_probe_supported_formats (GstObject * obj, snd_pcm_t * handle, const GstCaps * template_caps)    
#endif
{
...
if(endianness ==BIG_ENDIAN){ 
	if (!(caps = gst_alsa_detect_formats_be (obj, hw_params, caps))){
	    	goto subroutine_error;
	}
}else if(endianness ==LITTLE_ENDIAN){
	if (!(caps = gst_alsa_detect_formats_le (obj, hw_params, caps))){
	    	goto subroutine_error;
	}
}
#else
...
#if (defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)  
	system("rm -f /var/gstreamer-0.10/input_gstcaps");
#endif	
  return caps;
