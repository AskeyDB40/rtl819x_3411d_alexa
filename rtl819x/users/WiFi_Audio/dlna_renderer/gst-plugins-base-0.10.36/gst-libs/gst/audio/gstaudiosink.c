...
audioringbuffer_thread_func (GstRingBuffer * buf)
{
  ...
  setpriority(PRIO_PROCESS, 0, sink->priority_value);
  while (TRUE) {
    gint left, len;
    guint8 *readptr;
    gint readseg;
...
gst_audioringbuffer_acquire (GstRingBuffer * buf, GstRingBufferSpec * spec)
{
  ...
  spec->enable_multi_room = FALSE; // default
  if (csink->prepare)
    result = csink->prepare (sink, spec);
  if (!result)
    goto could_not_prepare;

  if (spec->enable_multi_room == TRUE)
      sink->priority_value = 10;
