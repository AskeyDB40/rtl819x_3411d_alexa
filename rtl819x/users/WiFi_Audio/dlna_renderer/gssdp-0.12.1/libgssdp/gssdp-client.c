                            ...
                            _GSSDPMessageType type)
{
        ...
        if (res == -1) {
	        	...
	        	if(type ==_GSSDP_DISCOVERY_RESPONSE){
	        		res = g_socket_send_to (socket,address, message,strlen (message),NULL, &error);
	        		 if (res == -1) {
	        		 	g_warning ("Error re-sending 1 SSDP packet to %s: %s",dest_ip,error->message);
	        		 	g_error_free (error);
	        		}else{
	        			if(type ==_GSSDP_DISCOVERY_RESPONSE){
	        				res = g_socket_send_to (socket,address, message,strlen (message),NULL, &error);
	        				 if (res == -1) {
	        		 			g_warning ("Error re-sending 2 SSDP packet to %s: %s",dest_ip,error->message);
	        		 			g_error_free (error);
	        		 		}
	        			}
	        		}
