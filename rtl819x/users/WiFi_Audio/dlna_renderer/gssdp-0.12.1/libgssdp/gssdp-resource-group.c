                                    ...
                                    gboolean            available)
{
        ...
        if (available) {
				...
				timeout=15; //use 15s the first announce
			...
			resource_group->priv->rollback_maxage==0;
...
resource_group_timeout (gpointer user_data)
{
        ...
        send_announcement_set (resource_group->priv->resources,
                               (GFunc) resource_alive);
	if(resource_group->priv->rollback_maxage != 3){
		if(resource_group->priv->rollback_maxage == 0)
		{
			timeout = 25;
			resource_group->priv->rollback_maxage =1;
		}else if(resource_group->priv->rollback_maxage ==1){
			timeout = 35;
			resource_group->priv->rollback_maxage =2;
		}else if(resource_group->priv->rollback_maxage ==2){
			timeout = resource_group->priv->max_age/4;
			resource_group->priv->rollback_maxage =3;
		}
		g_source_destroy (resource_group->priv->timeout_src);
     	//g_print("%s, %d Change SSDP Timeout to %ds\n",__FUNCTION__, __LINE__,timeout);
		resource_group->priv->timeout_src =g_timeout_source_new_seconds (timeout);
		g_source_set_callback (resource_group->priv->timeout_src, resource_group_timeout, resource_group, NULL);
		g_source_attach (resource_group->priv->timeout_src, g_main_context_get_thread_default ());
		g_source_unref (resource_group->priv->timeout_src);
	}
                     ...
                     gpointer            user_data)
{
        ...
        if (!target) {
        	if(from_ip)
                g_warning ("Discovery request did not have an ST header from %s",from_ip);
          else
        ...
        {
        	if(!strcmp(target ,"roku:ecp")){
        		//g_warning ("DONT Support Roku Discovery request");
        		return;
        	}
        }
        ...
        if (!mx_str || atoi (mx_str) <= 0) {
        	if(from_ip)
                g_warning ("Discovery request did not have a valid MX header from %s",from_ip);
 		else
        ...
        if (!man) {
        		if(from_ip)
                g_warning ("Discovery request did not have a valid MAN header from %s",from_ip);
			else
