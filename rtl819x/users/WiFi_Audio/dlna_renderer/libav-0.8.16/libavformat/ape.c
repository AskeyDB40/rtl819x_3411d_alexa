...
static int ape_read_header(AVFormatContext * s, AVFormatParameters * ap)
{
    ...
    s->duration = ape->totalsamples/ape->samplerate;
    ...
    for (i = 0; i < ape->totalframes; i++) {
        ...
        current_pts = current_pts+ape->frames[i].nblocks;
        pts = current_pts/ape->samplerate;
    }
...
static int ape_read_packet(AVFormatContext * s, AVPacket * pkt)
{
    ...
    avio_seek (s->pb, ape->frames[ape->currentframe].pos, SEEK_SET);
		...
		pos = s->pb->pos - (s->pb->write_flag ? 0 : (s->pb->buf_end - s->pb->buffer));
		offset1 = ape->frames[ape->currentframe].pos - pos;
	
	    if( offset1 >= 0 && offset1 <= (s->pb->buf_end - s->pb->buffer)){
			s->pb->buf_ptr = s->pb->buffer + offset1;
	    }
	    else{
	    	s->pb->seek(s->pb->opaque, ape->frames[ape->currentframe].pos, SEEK_SET);
	    }
	}
