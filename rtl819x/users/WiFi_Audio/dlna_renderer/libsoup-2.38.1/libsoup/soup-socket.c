...

{
	...
	if (!priv->gsock)
		goto cant_listen;
	finish_socket_setup (priv);
	g_socket_setopt_tcp (priv->gsock );
	/* Bind */
 ...
 **/
SoupAddress *
soup_socket_get_remote_ip_address (SoupSocket *sock, char *dest)
{
	SoupSocketPrivate *priv;
	GSocketAddress *addr;
	struct sockaddr_storage sa;
	...
	g_mutex_lock (&priv->addrlock);
		...
		sa_len = g_socket_address_get_native_size (addr);
		if(g_socket_address_to_native (addr, &sa, sa_len, NULL)){
			sock_addr = (struct sockaddr *)&sa;
			sock_addr_in = (struct sockaddr_in *)sock_addr;
			sprintf(dest, "%s",inet_ntoa(sock_addr_in->sin_addr));
			//g_debug("%s %d ip:%s",__FUNCTION__, __LINE__,dest); 
		}else{
			dest[0]='\0';
			g_debug("%s %d NOT GOT IP",__FUNCTION__, __LINE__);
		}
		g_object_unref (addr);
	g_mutex_unlock (&priv->addrlock);
