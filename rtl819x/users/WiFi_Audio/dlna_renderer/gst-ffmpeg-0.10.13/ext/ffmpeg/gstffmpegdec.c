...
extern gint64 export_Current_Demux_ts;
static gboolean
      ...
      switch (fmt) {
        ...
        case GST_FORMAT_BYTES:
        {
          ...
          stop = -1;
			...
			if(ffmpegdec->context->codec && ffmpegdec->context->codec->id == CODEC_ID_APE&& start > 0 && export_Current_Demux_ts != -1){
				start=export_Current_Demux_ts;
				time = start;
			}
...
gst_ffmpegdec_chain (GstPad * pad, GstBuffer * inbuf)
{
  ...
  in_offset = GST_BUFFER_OFFSET (inbuf);
	if (ffmpegdec->context->codec->id == CODEC_ID_APE && GST_CLOCK_TIME_IS_VALID (ffmpegdec->next_out)) {
		in_timestamp = ffmpegdec->next_out;
	}
