...
GstObject *parent_object=NULL;
  ...
  if (caps == NULL) {
    if (bclass->get_caps){
#if 0//(defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)    	 
    	 parent_object = gst_object_get_parent (GST_OBJECT (pad));
#endif
      caps = bclass->get_caps (bsink);
#if 0//(defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)
      if(parent_object){ //audiosink-actual-sink-alsa

	
	if (access("/var/gstreamer-0.10/bypass",R_OK) == 0 && access(AUDIO_SRC_CAPS_FILE,R_OK) == 0 && !strcmp(parent_object->name, "audiosink-actual-sink-alsa")) 
	{
		...
		int caps_signed, caps_channels;
			...
			if (fp) {
				...
				fclose(fp);
				if(caps){
					//g_print("<<<<<<<<<%s, %d BEFORE get bsink caps=%s\n\n", __FUNCTION__, __LINE__,gst_caps_to_string(caps));
					if(gst_caps_get_size (caps) == 1 ){
						caps  =  gst_caps_make_writable (caps);
						s = gst_caps_get_structure (caps, 0);
						if(s != NULL){
							gst_structure_set (s, "rate", G_TYPE_INT, sample_rate, NULL);
							gst_structure_set (s, "channels", G_TYPE_INT, caps_channels, NULL);
							gst_structure_set (s, "width", G_TYPE_INT, bit_width, NULL);
							gst_structure_set (s, "depth", G_TYPE_INT, bit_depth, NULL);
							if(caps_signed ==1)
								gst_structure_set (s, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
							else
								gst_structure_set (s, "signed", G_TYPE_BOOLEAN, FALSE, NULL);
   			...
   			//g_print("<<<<<<<<<%s, %d AFTER get bsink caps=%s\n\n", __FUNCTION__, __LINE__,gst_caps_to_string(caps));
		}else{
			//g_print("%s, %d \n", __FUNCTION__, __LINE__);
		}
