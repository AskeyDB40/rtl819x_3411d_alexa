...
#define AUDIO_SRC_CAPS_FILE "/var/gstreamer-0.10/gstcaps"
    ...
    GstCaps * out)
{
  gboolean ret = TRUE;
  GstBaseTransformClass *klass;
#if 0 //(defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)
GstObject *parent_object=NULL;
#endif
  klass = GST_BASE_TRANSFORM_GET_CLASS (trans);
...
#if 0 //(defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)) && defined(CONFIG_SND_RTL819XD_SOC_I2S)
//g_print("%s, %d have_same_caps: %d\n", __FUNCTION__, __LINE__,trans->have_same_caps);

parent_object = gst_object_get_parent (GST_OBJECT (trans->sinkpad));

if(parent_object){
	//g_print("%s, %d parent obj name=%s\n", __FUNCTION__, __LINE__, parent_object->name);

	if (access("/var/gstreamer-0.10/bypass",R_OK) == 0 && (!strcmp(parent_object->name, "conv") || !strcmp(parent_object->name, "resample") 
		|| !strcmp(parent_object->name,"audioconvert0") || !strcmp(parent_object->name,"audioresample0"))){
		if(trans->have_same_caps == 0){
			trans->have_same_caps = 1;
			//g_print("%s, %d FORCE have_same_caps: %d\n", __FUNCTION__, __LINE__,trans->have_same_caps);
		}
	}
	
 gst_object_unref (parent_object);
}
#endif
  ...
  GST_DEBUG_OBJECT (pad, "have new caps %p %" GST_PTR_FORMAT, caps, caps);
	...
	if(gst_caps_get_size (caps) == 1 ){
		...
		if(s != NULL){
			...
			value = gst_structure_get_value (s, "rate");
			if (G_VALUE_TYPE (value) == G_TYPE_INT) {
		    		ret = gst_structure_get (s, "rate", G_TYPE_INT, &sample_rate, NULL);
			}else
				sample_rate = 0;
			...
			if (G_VALUE_TYPE (value) == G_TYPE_INT) {
				...
				byte_endianness = 2;	 ///unknow endian
				
			value = gst_structure_get_value (s, "width");
			if (G_VALUE_TYPE (value) == G_TYPE_INT) {
		    		ret = gst_structure_get (s, "width", G_TYPE_INT, &bit_width, NULL);
			}else
				bit_width = 0;	
				
			value = gst_structure_get_value (s, "depth");
			if(value != NULL){
				if (G_VALUE_TYPE (value) == G_TYPE_INT) {
			    		ret = gst_structure_get (s, "depth", G_TYPE_INT, &bit_depth, NULL);
				}else
					bit_depth = 0;	
			}else{
				//g_print("%s, %d CAN NOT GET depth\n", __FUNCTION__, __LINE__);
				bit_depth = bit_width;
			}
			
			value = gst_structure_get_value (s, "channels");
			if (G_VALUE_TYPE (value) == G_TYPE_INT) {
		    		ret = gst_structure_get (s, "channels", G_TYPE_INT, &caps_channels, NULL);
			}else
				caps_channels = 0;	
			
			value = gst_structure_get_value (s, "signed");
			if(value != NULL){
				if (G_VALUE_TYPE (value) == G_TYPE_BOOLEAN) {
			    		ret = gst_structure_get (s, "signed", G_TYPE_BOOLEAN, &caps_signed, NULL);
				}else
					caps_signed = 0;
			}else{
				//g_print("%s, %d CAN NOT GET signed\n", __FUNCTION__, __LINE__);
					caps_signed = 1;
				}
		}
	}
	{
	//g_print("\n<<<%s, %d rate=%d, endian=%d, width=%d signed=%d>>>>>\n", __FUNCTION__, __LINE__,sample_rate, byte_endianness,bit_width, caps_signed);
		//if (access(AUDIO_SRC_CAPS_FILE,R_OK) < 0 && sample_rate > 48000 && sample_rate != 96000 && bit_width ==16) {
		if (access(AUDIO_SRC_CAPS_FILE,R_OK) < 0 && sample_rate > 8000 && bit_width ==16) {
		//if (access(AUDIO_SRC_CAPS_FILE,R_OK) < 0 && bit_width ==16) {	
			char cmdData[128]={0};
			if(byte_endianness == G_BYTE_ORDER)
				byte_endianness = 1;
			else if(byte_endianness == G_LITTLE_ENDIAN)
				byte_endianness = 0;
			
//			g_print("%s, %d Put caps to /var/gstreamer-0.10/gstcaps\n",__FUNCTION__, __LINE__);
			sprintf(cmdData, "%d %d %d %d %d %d", sample_rate,  caps_channels, caps_signed, byte_endianness, bit_width, bit_depth);
			write_line_to_file("/var/gstreamer-0.10/gstcaps", 1, cmdData);
			sprintf(cmdData, "%s", "1");
			write_line_to_file("/var/gstreamer-0.10/bypass", 1, cmdData);
		}
	}
#endif
